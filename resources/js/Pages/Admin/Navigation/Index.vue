    ...
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div v-for="(items, location) in { Header: headerItems, Footer: footerItems }" :key="location">
                ...
                <div class="space-y-2 mb-4">
                    <div v-for="item in items" :key="item.id" class="bg-black-warm border border-gold-deep/20 rounded p-4">
                        ...
                        <!-- Macedonian label override -->
                        <div class="mt-3 flex items-center gap-2">
                            <span class="text-warm-grey/60 text-[9px] uppercase tracking-[0.2em] font-semibold shrink-0">MK</span>
                            <input
                                v-model="mkLabels[item.id]"
                                :placeholder="item.label"
                                class="flex-1 bg-transparent border border-gold-deep/30 text-off-white text-xs px-2 py-1 rounded focus:outline-none focus:border-gold transition-colors"
                            >
                            <button
                                @click="saveMk(item)"
                                class="px-3 py-1 text-[9px] uppercase tracking-[0.2em] font-semibold border border-gold-deep/30 text-gold hover:bg-gold/10 rounded transition-colors"
                            >Save</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-8 bg-black-warm border border-gold-deep/20 rounded p-6 max-w-lg">
            ...
            <form @submit.prevent="addItem" class="space-y-3">
                ...
                <input v-model="newItem.label" placeholder="Label (English)" class="w-full bg-transparent border border-gold-deep/30 text-off-white text-sm px-3 py-2 rounded focus:outline-none focus:border-gold transition-colors">
                <input v-model="newItem.label_mk" placeholder="Ознака (Македонски) — optional" class="w-full bg-transparent border border-gold-deep/30 text-off-white text-sm px-3 py-2 rounded focus:outline-none focus:border-gold transition-colors">
...
const props = defineProps({ headerItems: Array, footerItems: Array });

// Per-item Macedonian label drafts, keyed by item id.
const mkLabels = reactive({});
[...(props.headerItems || []), ...(props.footerItems || [])].forEach((item) => {
    mkLabels[item.id] = item.label_mk ?? '';
});

const newItem = reactive({ location: 'header', label: '', label_mk: '', url: '', is_cta: false, is_external: false, sort_order: 99 });

function addItem() {
    router.post('/admin/navigation', {
        ...newItem,
        translations: { mk: { label: newItem.label_mk } },
    }, { preserveScroll: true });
    newItem.label = '';
    newItem.label_mk = '';
    newItem.url = '';
...
function saveMk(item) {
    router.put(`/admin/navigation/${item.id}`, {
        label: item.label,
        url: item.url,
        is_cta: !!item.is_cta,
        ...
        translations: { mk: { label: mkLabels[item.id] } },
    }, { preserveScroll: true });
}
